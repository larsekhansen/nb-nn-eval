...
- [ ] **Post-translation consistency checker.** Scan a translated document and flag:
  ...
  **Two-phase design:**
  - **Phase 1: Rule-based detection (free, fast).** A Python script that tokenizes source and translation, builds a term-alignment table, and flags: (a) any bokmål term that maps to 2+ different nynorsk translations across the document, (b) any word that appears unchanged in both versions (potential untranslated fragment), (c) known conservative/radical nynorsk form pairs appearing in the same document (style drift). This catches ~70% of issues with zero API cost and could live in this repo.
  - **Phase 2: LLM judgement pass (cheap, accurate).** Send only the flagged segments to Claude or GPT-4o to filter false positives and explain each real issue: "is this actually an inconsistency, or are both forms valid nynorsk?" Keeps API costs minimal because you're sending 10-20 flagged pairs, not the entire document. Uses the API adapters we already have. NorMistral-11b is the strongest open-source option for nynorsk understanding (per Språkrådet 2025), but lacks the structured reasoning ability of the larger closed-source models for this specific task.
